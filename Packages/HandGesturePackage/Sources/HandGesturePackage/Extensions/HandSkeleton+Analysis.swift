...
extension HandSkeleton {
    ...
    func isPalmFacingAway(tolerance: Float = defaultTolerance) -> Bool {
        return palmNormal.isFacing(.away, tolerance: tolerance)
    }

    /// 手のひらが手前を向いているかを判定
    func isPalmFacingToward(tolerance: Float = defaultTolerance) -> Bool {
        return palmNormal.isFacing(.toward, tolerance: tolerance)
    }
...
extension SIMD3<Float> {
    ...
    func palmDirection(tolerance: Float) -> PalmDirection {
        ...
        let directions: [PalmDirection] = [.up, .down, .right, .left, .toward, .away]
...
extension HandSkeleton {
    ...
    /// 手のひらの方向
    enum PalmDirection {
        case up, down, left, right, toward, away
        ...
        var vector: SIMD3<Float> {
            ...
            case .left: return SIMD3<Float>(-1, 0, 0)
            case .toward: return SIMD3<Float>(0, 0, -1)
            case .away: return SIMD3<Float>(0, 0, 1)
            }
        }
    }
